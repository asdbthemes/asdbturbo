...
class asdb_menu {
    ...
    function hook_wp_edit_nav_menu_walker () {
        include_once('class-asdb-mega-menu-back.php');
        return 'asdb_nav_menu_edit_walker';
    }

    function hook_wp_update_nav_menu_item ($menu_id, $menu_item_db_id, $args) {

        //echo $menu_item_db_id;
        if (isset($_POST['asdb_mega_menu_cat'][$menu_item_db_id])) {
            //print_r($_POST);
            update_post_meta($menu_item_db_id, 'asdb_mega_menu_cat', $_POST['asdb_mega_menu_cat'][$menu_item_db_id]);
            update_post_meta($menu_item_db_id, 'asdb_mega_menu_page_id', $_POST['asdb_mega_menu_page_id'][$menu_item_db_id]);
        }
    ...
    function hook_wp_nav_menu_objects($items, $args = '') {
        ...
        foreach ($items as &$item) {
            ...
            // first menu fix
            if ($asdb_is_firstMenu) {
                $item->classes[] = 'menu-item-first';
                $asdb_is_firstMenu = false;
            }
            ...
            if ($this->is_header_menu_mobile === true) {
                ...
                if ($asdb_mega_menu_cat != '') {
                    ...
                    $asdb_render_atts = asdb_api_block::get_key('asdb_block_mega_menu', 'render_atts');

                    $asdb_show_child_cat = 5;
                    if (isset($asdb_render_atts['show_child_cat'])) {
                        $asdb_show_child_cat = $asdb_render_atts['show_child_cat'];
                    }

                    //check for subcategories
                    $asdb_subcategories = get_categories(array(
                        'child_of' => $asdb_mega_menu_cat,
                        'number' => $asdb_show_child_cat  //
                    ));
                    if (!empty($asdb_subcategories)) {
                        $item->classes[] = 'menu-item-has-children'; // add the extra class for the dropdown to work
                        foreach ($asdb_subcategories as $asdb_category) {
                            ...
                            $new_item->url = get_category_link($asdb_category->cat_ID);
                            $new_item->title = $asdb_category->name;
            ...
            elseif ($asdb_mega_menu_page_id != '') {
                ...
                $content_post = get_post($asdb_mega_menu_page_id);
            ...
            elseif ($asdb_mega_menu_cat != '') {
                ...
                $new_item->title .= asdb_global_blocks::get_instance('asdb_block_mega_menu')->render(
                    array(
                        'limit' => '5',
                        'asdb_column_number' => 3,
                        'ajax_pagination' => 'next_prev',
                        'category_id' => $asdb_mega_menu_cat,
                        'show_child_cat' => $asdb_render_atts['show_child_cat'],
                        'asdb_ajax_filter_type' => 'asdb_category_ids_filter',
                        'asdb_ajax_preloading' => asdb_util::get_option('tds_mega_menu_ajax_preloading')
                    ));
                $new_item->title .= '</div></div>';
                $items_buffy[] = $new_item;
            }
...
class asdb_tagdiv_walker_nav_menu extends Walker_Nav_Menu {
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        //tagdiv - megamenu disable link from from includes/wp_booster/asdb_menu.php  hook_wp_nav_menu_objects
        ...
        //tagdiv - megamenu disable link from includes/wp_booster/asdb_menu.php   hook_wp_nav_menu_objects
        if ($item->is_mega_menu == false) {
            $item_output .= '</a>';
        }
